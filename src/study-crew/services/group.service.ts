...
try {
return await this.groupModel.create(updatedData as any, { userId } as CreateOptions<any>);
} catch (error) {
...
async findAll(search?: string, offset?: number, limit?: number): Promise<{rows: Group[], count: number}> {
try {
const where: any = {};
if (search) {
where[Op.or] = [
...
avatar?: Express.Multer.File,): Promise<Group> {
  let avatarUrl: string | null = null;
...
return await this.userGroupModel.bulkCreate(records, { ignoreDuplicates: true });
} catch (error) {
throw new BadRequestException({
...
try {
return await this.userGroupModel.destroy({ where: { userId, groupId: groupIds } });
} catch (error) {
throw new BadRequestException({
...
async updateUserGroupStatus(userId: string, groupId: string, status: 'ACCEPTED' | 'DECLINED' | 'PENDING'): Promise<UserGroup> {
try {
const userGroup = await this.userGroupModel.findOne({ where: { userId, groupId } });
if (!userGroup) {
throw new NotFoundException(`UserGroup link not found for userId ${userId} and groupId ${groupId}`);
}
