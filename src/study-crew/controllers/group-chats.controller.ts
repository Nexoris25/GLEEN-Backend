...
import { GroupChatResponseCountDto, GroupChatResponseDto, ResponseDto } from 'src/shared-types/response.dto';
...
export class GroupChatsController {
  ...
  async create(@Body() createGroupChatDto: CreateGroupChatDto, @UserId() userId: string): Promise<GroupChatResponseDto> {
    try {
      const chat = await this.groupChatsService.create(createGroupChatDto, userId);
      return {
  ...
  @Get()
  @ApiOperation({ summary: 'Get all group chat messages (optionally by group)' })
  ...
  @ApiResponse({ status: 200, description: 'Group chats retrieved successfully', type: GroupChatResponseCountDto })
  @ApiResponse({ status: 500, description: 'Error retrieving group chats' })
  async findAll(@Query('groupId') groupId?: string, @Query('offset') offset?: number, @Query('limit') limit?: number): Promise<GroupChatResponseCountDto> {
    try {
      const chats = await this.groupChatsService.findAll(groupId, offset, limit);
      return {
  ...
  async update(@Param('id') id: string, @Body() updateGroupChatDto: UpdateGroupChatDto): Promise<GroupChatResponseDto> {
    try {
