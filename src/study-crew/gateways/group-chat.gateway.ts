...
import { JoinRoomDto, LeaveRoomDto, SendMessageDto, MarkAsReadDto } from '../dto/group-chat-gateway.dto';
import { GroupChatsService } from '../services/group-chats.service';

@WebSocketGateway({
    namespace: '/group-chat', cors: {
        origin: '*',
    },
})
export class GroupChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleMarkAsRead(@MessageBody() data: MarkAsReadDto, @ConnectedSocket() client: Socket) {
        await this.groupChatService.markAsRead(data.messageId);
        this.server.to(data.groupId).emit('messageRead', { messageId: data.messageId, userId: data.userId });
        return { event: 'markedAsRead', messageId: data.messageId, userId: data.userId };
    }
}
