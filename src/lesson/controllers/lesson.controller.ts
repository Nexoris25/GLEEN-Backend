...
import { Body, Controller, Delete, Get, Param, Post, Put, Patch, Query, UseGuards, UploadedFile,
  UseInterceptors,  BadRequestException, ValidationPipe, Req, ParseUUIDPipe } from '@nestjs/common';
import { ApiQuery, ApiBody, ApiOperation, ApiParam, ApiResponse, ApiTags, ApiBearerAuth, ApiConsumes, } from '@nestjs/swagger';
...
import { LessonArrayResponseCountDto, LessonCommentResponseCountDto, LessonCommentResponseDto, LessonResponseDto, LessonTrackingResponseDto, ResponseDto } from 'src/shared-types/response.dto';
...
@ApiResponse({ status: 500, description: 'Error creating lesson', type: LessonResponseDto })
async create(@Body() createLessonDto: CreateLessonDto, @UserId() userId: string, @UploadedFile() avatarOrCover: Express.Multer.File,) {
try {
const lesson = await this.lessonService.create(createLessonDto, userId, avatarOrCover);
return { status: 201, data: lesson, message: 'Lesson created successfully' };
} catch (error) {
return {
status: 500,
...
@ApiResponse({ status: 404, description: 'Lesson not found', type: LessonResponseDto })
@ApiResponse({ status: 500, description: 'Error updating lesson', type: LessonResponseDto })
async update(
  @Param('id') id: string,
  @Body(
    new ValidationPipe({
  ...
  try {
    ...
    const lesson = await this.lessonService.update(id, updateLessonDto, avatarOrCover);
    return { status: 200, data: lesson, message: 'Lesson updated successfully' };
  } catch (error) {
    return {
      status: 500,
...
@ApiParam({ name: 'id', description: 'Lesson ID' })
@ApiResponse({ status: 200, description: 'Lesson deleted successfully', schema: { example: { status: 200, message: 'Lesson deleted successfully' } } })
@ApiResponse({ status: 404, description: 'Lesson not found', type: LessonResponseDto })
@ApiResponse({ status: 500, description: 'Error deleting lesson', type: LessonResponseDto })
...
@ApiResponse({ status: 201, description: 'Lesson comment created successfully', type: LessonCommentResponseDto })
@ApiResponse({ status: 404, description: 'Lesson not found', type: ResponseDto })
@ApiResponse({ status: 500, description: 'Error creating lesson comment', type: ResponseDto })
async createComment(@Body() createCommentDto: CreateLessonCommentDto, @UserId() userId: string): Promise<LessonCommentResponseDto> {
try {
const comment = await this.lessonService.createComment(createCommentDto, userId);
return { status: 201, data: comment, message: 'Lesson comment created successfully' };
} catch (error) {
return {
status: 500,
...
@ApiResponse({ status: 200, description: 'Lesson comment retrieved successfully', type: LessonCommentResponseDto })
@ApiResponse({ status: 404, description: 'Lesson comment not found', type: ResponseDto })
@ApiResponse({ status: 500, description: 'Error retrieving lesson comment', type: ResponseDto })
...
return { status: 200, data: comment, message: 'Lesson comment retrieved successfully' };
} catch (error) {
return {
status: 500,
...
@ApiResponse({ status: 200, description: 'Lesson comments retrieved successfully', type: LessonCommentResponseCountDto})
@ApiResponse({ status: 404, description: 'Lesson not found', type: ResponseDto })
@ApiResponse({ status: 500, description: 'Error retrieving lesson comments', type: ResponseDto })
async getCommentsByLesson(@Param('lessonId') lessonId: string): Promise<LessonCommentResponseCountDto> {
try {
const comments = await this.lessonService.findCommentByLesson(lessonId);
return { status: 200, data: comments, message: 'Lesson comments retrieved successfully' };
} catch (error) {
return {
status: 500,
...
try {
const comment = await this.lessonService.findCommentByLessonAndUser(lessonId, userId);
return { status: 200, data: comment, message: 'Lesson comment retrieved successfully' };
} catch (error) {
return {
status: 500,
...
@ApiResponse({ status: 200, description: 'Lesson comment updated successfully', type: LessonCommentResponseDto })
@ApiResponse({ status: 404, description: 'Lesson comment not found', type: ResponseDto })
@ApiResponse({ status: 500, description: 'Error updating lesson comment', type: ResponseDto })
async updateComment(@Param('id') id: string, @Body() updateCommentDto: UpdateLessonCommentDto): Promise<LessonCommentResponseDto> {
try {
const comment = await this.lessonService.updateComment(id, updateCommentDto);
return { status: 200, data: comment, message: 'Lesson comment updated successfully' };
} catch (error) {
return {
status: 500,
...
@ApiParam({ name: 'id', description: 'Comment ID' })
@ApiResponse({ status: 200, description: 'Lesson comment deleted successfully', schema: { example: { status: 200, message: 'Lesson comment deleted successfully' } } })
@ApiResponse({ status: 404, description: 'Lesson comment not found', type: ResponseDto })
@ApiResponse({ status: 500, description: 'Error deleting lesson comment', type: ResponseDto })
async removeComment(id: string): Promise<ResponseDto<null>> {
try {
await this.lessonService.removeComment(id);
return { status: 200, data: null, message: 'Lesson comment deleted successfully' };
} catch (error) {
return {
status: 500,
...
async completeLesson(
  ...
  try {
    ...
    const updatedTracking =
      await this.lessonService.completeLessonTracking(
        lessonId,
        userId,
      );
