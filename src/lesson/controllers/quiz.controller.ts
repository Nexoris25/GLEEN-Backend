...
export class QuizController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Quiz comment created successfully',
    type: QuizCommentResponseDto,
  })
  @ApiResponse({
    status: 404,
    description: 'Quiz not found',
  ...
  @ApiResponse({
    status: 500,
    description: 'Error creating quiz comment',
    type: ResponseDto,
  })
  async createComment(
    @Body() createCommentDto: CreateQuizCommentDto,
    @UserId() userId: string,
  ): Promise<QuizCommentResponseDto> {
    try {
      const comment = await this.quizzesService.createComment(
        createCommentDto,
        userId,
      );
      return {
        status: 201,
        data: comment,
        message: 'Quiz comment created successfully',
      };
    } catch (error) {
      return {
        status: 500,
  ...
  @ApiResponse({
    status: 200,
    description: 'Quiz comment retrieved successfully',
    type: QuizCommentResponseDto,
  })
  @ApiResponse({
    status: 404,
    description: 'Quiz comment not found',
    type: ResponseDto,
  })
  @ApiResponse({
    status: 500,
    description: 'Error retrieving quiz comment',
    type: ResponseDto,
  })
  async getComment(@Param('id') id: string): Promise<QuizCommentResponseDto> {
    try {
      ...
      return {
        status: 200,
        data: comment,
        message: 'Quiz comment retrieved successfully',
      };
    } catch (error) {
      return {
        status: 500,
  ...
  @ApiResponse({
    status: 200,
    description: 'Quiz comments retrieved successfully',
    type: QuizCommentResponseCountDto,
  })
  @ApiResponse({
    status: 404,
    description: 'Quiz not found',
  ...
  @ApiResponse({
    status: 500,
    description: 'Error retrieving quiz comments',
    type: ResponseDto,
  })
  async getCommentsByQuiz(
    @Param('quizId') quizId: string,
    @Query('offset') offset: number,
    @Query('limit') limit: number,
  ): Promise<QuizCommentResponseCountDto> {
    try {
      const comments = await this.quizzesService.findCommentByQuiz(
        quizId,
        offset,
        limit,
      );
      return {
        status: 200,
        data: comments,
        message: 'Quiz comments retrieved successfully',
      };
    } catch (error) {
      return {
        status: 500,
  ...
  async getCommentByQuizAndUser(
    ...
    try {
      const comment = await this.quizzesService.findCommentByQuizAndUser(
        quizId,
        userId,
      );
      return {
  ...
  @ApiResponse({
    status: 200,
    description: 'Quiz comment updated successfully',
    type: QuizCommentResponseDto,
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiResponse({
    status: 500,
    description: 'Error updating quiz comment',
    type: ResponseDto,
  })
  async updateComment(
    @Param('id') id: string,
    @Body() updateCommentDto: UpdateQuizCommentDto,
  ): Promise<QuizCommentResponseDto> {
    try {
      const comment = await this.quizzesService.updateComment(
        id,
        updateCommentDto,
      );
      return {
  ...
  @ApiResponse({
    status: 200,
    description: 'Quiz comment deleted successfully',
    schema: {
      example: { status: 200, message: 'Quiz comment deleted successfully' },
    },
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiResponse({
    status: 500,
    description: 'Error deleting quiz comment',
    type: ResponseDto,
  })
  async removeComment(id: string): Promise<ResponseDto<null>> {
    try {
      ...
      return {
        status: 200,
        data: null,
        message: 'Quiz comment deleted successfully',
      };
    } catch (error) {
      return {
        status: 500,
