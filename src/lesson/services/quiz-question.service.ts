...
export class QuizQuestionsService {
    ...
    async create(createDto: CreateQuizQuestionDto, userId: string): Promise<QuizQuestions> {
        try {
            return await this.quizQuestionsModel.create(createDto as Omit<QuizQuestions, 'id'>, { isNewRecord: true, userId });
        } catch (error) {
    ...
    async findAll(options: SearchQuizQuestionDto): Promise<{ rows: QuizQuestions[]; count: number }> {
        try {
    ...
    async update(id: string, updateDto: UpdateQuizQuestionDto): Promise<QuizQuestions | null> {
        try {
    ...
    async delete(id: string): Promise<boolean> {
        try {
            const deletedCount = await this.quizQuestionsModel.destroy({ where: { id } });
