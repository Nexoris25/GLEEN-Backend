...
export class LessonService {
  ...
  async findTrackingByLessonAndUser(
    lessonId: string,
    userId: string,
  ...
  async updateTracking(
    id: string,
    updateTrackingDto: UpdateLessonTrackingDto,
  ): Promise<LessonTracking> {
    try {
  ...
  async findCommentByLesson(
    lessonId: string,
  ): Promise<{ rows: LessonComment[]; count: number }> {
    try {
      const comments = await this.lessonCommentModel.findAndCountAll({
        where: { lessonId },
      });
      return comments;
    } catch (error) {
  ...
  async findCommentByLessonAndUser(
    lessonId: string,
    userId: string,
  ): Promise<LessonComment | null> {
    try {
      const comment = await this.lessonCommentModel.findOne({
        where: { lessonId, userId },
      });
      return comment;
    } catch (error) {
  ...
  async updateComment(
    id: string,
    updateCommentDto: UpdateLessonCommentDto,
  ): Promise<LessonComment> {
    try {
  ...
  async findAll(
    ...
    try {
      ...
      const lesson = await this.lessonModel.findAndCountAll({
        where: whereClause,
        offset,
        limit,
      });
