...
export class StudentsQuizAnswersService {
    ...
    async create(answerData: { quizQuestionId: string; answer: string }, userId: string): Promise<StudentsQuizAnswers> {
        try {
            const { quizQuestionId, answer } = answerData;

            const quizQuestion = await this.quizQuestionService.findById(quizQuestionId);

            if (!quizQuestion) {
                throw new Error("Quiz Question for the given Id Could not be found");
            }

            const score = quizQuestion.correctAnswer.toLowerCase() === answer.toLowerCase() || quizQuestion.correctAnswer === answer ? 1 : 0;
    ...
    async findAll(searchDto: SearchQuizAnswerDto): Promise<{ rows: StudentsQuizAnswers[], count: number }> {
        try {
    ...
    async findByQuizQuestionAndUser(quizQuestionId: string, userId: string): Promise<StudentsQuizAnswers | null> {
        try {
            return await this.studentsQuizAnswersModel.findOne({ where: { quizQuestionId, userId } });
        } catch (error) {
            throw new BadRequestException({
    ...
    async findByQuizAndUserId(quizId: string, userId: string): Promise<StudentsQuizAnswers[]> {
        try {
            return await this.studentsQuizAnswersModel.findAll({ where: { userId }, include: [{ association: 'quizQuestion', where: { quizId } }] });
        } catch (error) {
            throw new BadRequestException({
    ...
    async update(id: string, updateData: Partial<StudentsQuizAnswers>): Promise<StudentsQuizAnswers | null> {
        try {
            ...
            const score = quizQuestion.correctAnswer.toLowerCase() === updateData.answer.toLowerCase() || quizQuestion.correctAnswer === updateData.answer ? 1 : 0;
            const updatedAnswer = await answer.update({ ...updateData, score });
            // Compute and update quiz record after answer update
            await this.computeAndUpdateQuizRecord(answer.quizQuestionId, answer.userId);
     ...
     */
    private async computeAndUpdateQuizRecord(quizQuestionId: string, userId: string): Promise<void> {
        // Get the quiz question to find the quizId
        const quizQuestion = await this.studentsQuizAnswersModel.sequelize.models.QuizQuestions.findByPk(quizQuestionId);
        if (!quizQuestion) return;
        const quizId = quizQuestion.getDataValue('quizId');

        // Get all questions for the quiz
        const allQuestions = await this.studentsQuizAnswersModel.sequelize.models.QuizQuestions.findAll({ where: { quizId } });
        ...
        // Upsert quiz record
        const quizRecordModel = this.studentsQuizAnswersModel.sequelize.models.QuizRecord;
    ...
    async delete(id: string): Promise<boolean> {
        try {
            const deletedCount = await this.studentsQuizAnswersModel.destroy({ where: { id } });
