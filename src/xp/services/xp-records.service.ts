...
export class XpRecordsService {
    ...
    async create(createXpRecordsDto: CreateXpRecordsDto): Promise<XpRecords> {
        try {
            ...
            if (existingRecord) {
                throw new Error(`XP record already exists for user ${createXpRecordsDto.userId}. Use update instead.`);
            }
     ...
     */
    async updateByUserId(userId: string, updateXpRecordsDto: UpdateXpRecordsDto): Promise<XpRecords> {
        try {
            const xpRecord = await this.xpRecordsRepository.findOne({
                where: { userId }
            });
     ...
     */
    async getLeaderboard(leaderboardQueryDto?: LeaderboardQueryDto): Promise<XpRecords[]> {
        try {
            const { limit = 10 } = leaderboardQueryDto || {};
            const leaderboard = await this.xpRecordsRepository.findAll({
                include: [{ association: 'user', required: true, include: [{ association: 'lessons'}, { association: 'quizRecords'}] }],
