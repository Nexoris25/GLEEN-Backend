...
export class XpLogService {
     ...
     */
    async findAll(queryDto?: XpLogQueryDto): Promise<{ data: XpLog[]; total: number }> {
        try {
            const { page = 1, limit = 10, xpType, startDate, endDate } = queryDto || {};
     ...
     */
    async findByUserId(userId: string, queryDto?: XpLogQueryDto): Promise<{ data: XpLog[]; total: number }> {
        try {
            const { page = 1, limit = 10, xpType, startDate, endDate } = queryDto || {};
            const offset = (page - 1) * limit;
     ...
     */
    async getXpSummaryByUserId(userId: string): Promise<{ xpType: string; totalXp: number }[]> {
        try {
            const result = await this.xpLogRepository.findAll({
                where: { userId },
                attributes: [
                    'xpType',
                    [this.xpLogRepository.sequelize.fn('SUM', this.xpLogRepository.sequelize.col('xpValue')), 'totalXp']
                ],
                group: ['xpType'],
                raw: true
            });
