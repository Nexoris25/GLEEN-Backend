...
import { ComplaintArrayResponseCountDto, ComplaintResponseDto, CreateComplaintDto, UpdateComplaintDto } from '../dto/complaint.dto';
...
export class ComplaintController {
    ...
    async create(@Body() dto: CreateComplaintDto, @UserId() userId: string): Promise<ComplaintResponseDto> {
        try {
            const sub = await this.complaintService.create(dto, userId);
            return { status: HttpStatus.CREATED, message: 'Complaint created', data: sub };
        } catch (error) {
            return {
    ...
    @ApiQuery({ name: 'status', required: false, enum: ['open', 'in_progress', 'resolved'], description: 'Filter by complaint status' })
    ...
    @ApiResponse({ status: 200, description: 'List of complaints', type: ComplaintArrayResponseCountDto })
    async findAll(@Query() query: any) {
        try {
            const sub = await this.complaintService.findAll(query);
            return { status: HttpStatus.OK, message: 'Complaints retrieved successfully', data: sub };
        } catch (error) {
            return {
                status: HttpStatus.INTERNAL_SERVER_ERROR,
    ...
    @ApiResponse({ status: 200, description: 'Complaint found', type: ComplaintResponseDto })
    @ApiResponse({ status: 404, description: 'Complaint not found' })
    async findOne(@Param('id') id: string) {
        try {
            const sub = await this.complaintService.findOne(id);
            return { status: HttpStatus.OK, message: 'Complaint retrieved successfully', data: sub };
        } catch (error) {
            return {
                status: HttpStatus.INTERNAL_SERVER_ERROR,
    ...
    @ApiResponse({ status: 200, description: 'Complaint updated successfully', type: ComplaintResponseDto })
    async update(@Param('id') id: string, @Body() dto: UpdateComplaintDto) {
        try {
            const sub = await this.complaintService.update(id, dto);
            return { status: HttpStatus.OK, message: 'Complaint updated successfully', data: sub };
        } catch (error) {
            return {
                status: HttpStatus.INTERNAL_SERVER_ERROR, 
    ...
    @ApiParam({ name: 'id', description: 'Complaint ID', type: String })
    @ApiResponse({ status: 200, description: 'Complaint deleted successfully', type: ComplaintResponseDto })
    async remove(@Param('id') id: string) {
        try {
            const sub = await this.complaintService.remove(id);
            return { status: HttpStatus.OK, message: 'Complaint deleted successfully', data: sub };
        } catch (error) {
            return {
                status: HttpStatus.INTERNAL_SERVER_ERROR,
