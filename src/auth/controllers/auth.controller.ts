...
import { ApiResponse, ApiTags, ApiParam, ApiBody, ApiBearerAuth, ApiOperation } from '@nestjs/swagger';
...
const stateExists = await this.stateModel.findByPk(
createUserDto.stateId,
);
...
try {
  console.log(`'userId', ${user.id} OLD ${body.oldPassword} NEW ${body.newPassword}`);
