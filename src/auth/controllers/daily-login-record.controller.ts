...
export class DailyLoginRecordController {
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Start date (inclusive) filter, ISO string',
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'End date (inclusive) filter, ISO string',
    example: '2025-09-28T23:59:59.999Z',
  })
  @ApiOkResponse({
    ...
    schema: {
      ...
      properties: {
        data: {
          type: 'array',
          items: { $ref: getSchemaPath(DailyLoginRecord) },
        },
        count: { type: 'number' },
      },
    },
  ...
  @Get(':userId')
  @ApiOperation({
    summary: 'Get daily login records for a specific user (paginated)',
  })
  @ApiParam({
    name: 'userId',
    required: true,
    description: 'User UUID to fetch records for',
    type: String,
  ...
  getByUser(
    ...
    return this.dailyService.findByUser(
      userId,
      offset,
      limit,
      startDate,
