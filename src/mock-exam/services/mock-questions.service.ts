...
import { CreateBulkMockQuestionDto, CreateMockQuestionDto } from '../dtos/create-mock-questions.dto';
...
export class MockQuestionsService {
    ...
    async create(createDto: CreateMockQuestionDto, userId: string): Promise<MockQuestions> {
        try {
            return await this.mockQuestionsModel.create(createDto as Omit<MockQuestions, 'id'>, { isNewRecord: true, userId });
        } catch (error) {
    ...
    async findAll(options: SearchMockQuestionDto): Promise<{ rows: MockQuestions[]; count: number }> {
        try {
    ...
    async update(id: string, updateDto: UpdateMockQuestionDto): Promise<MockQuestions | null> {
        try {
    ...
    async delete(id: string): Promise<boolean> {
        try {
            const deletedCount = await this.mockQuestionsModel.destroy({ where: { id } });
