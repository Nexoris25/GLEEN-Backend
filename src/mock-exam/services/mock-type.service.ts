...
export class MockTypesService {
    ...
    async create(createDto: CreateMockTypeDto, userId: string): Promise<MockTypes> {
        try {
            return await this.mockTypesModel.create(createDto as Omit<MockTypes, 'id'>, { isNewRecord: true, userId });
        } catch (error) {
    ...
    async findAll(options: SearchMockTypeDto): Promise<{ rows: MockTypes[]; count: number }> {
        try {
    ...
    async update(id: string, updateDto: UpdateMockTypeDto): Promise<MockTypes | null> {
        try {
