...
export class StudentsMockAnswersService {
    ...
    async create(answerData: { mockQuestionId: string; answer: string, mockExamRecordId:string }, userId: string): Promise<StudentsMockAnswers> {
        try {
            const { mockQuestionId, mockExamRecordId, answer } = answerData;
            const mockQuestion = await this.mockQuestionService.findById(mockQuestionId);

            if (!mockQuestion) {
                throw new Error("Mock Question for the given Id Could not be found");
            }

            const score = mockQuestion.correctAnswer.toLowerCase() === answer.toLowerCase() || mockQuestion.correctAnswer === answer ? 1 : 0;
    ...
    async findAll(searchDto: SearchMockAnswerDto): Promise<{ count: number; rows: StudentsMockAnswers[] }> {
        try {
    ...
    async findByMockQuestionAndUser(mockQuestionId: string, userId: string): Promise<StudentsMockAnswers | null> {
        try {
            return await this.studentsMockAnswersModel.findOne({ where: { mockQuestionId, userId } });
        } catch (error) {
            throw new BadRequestException({
    ...
    async findByLessonAndUserId(mockExamId: string, userId: string): Promise<{ count: number; rows: StudentsMockAnswers[] }> {
        try {
            return await this.studentsMockAnswersModel.findAndCountAll({ where: { userId }, include: [{ association: 'mockQuestion', where: { mockExamId } }] });
        } catch (error) {
            throw new BadRequestException({
    ...
    async update(id: string, updateData: Partial<StudentsMockAnswers>): Promise<StudentsMockAnswers | null> {
        try {
            ...
            const mockQuestion = await this.mockQuestionService.findById(answer.mockQuestionId);
            ...
            const score = mockQuestion.correctAnswer.toLowerCase() === updateData.answer.toLowerCase() || mockQuestion.correctAnswer === updateData.answer ? 1 : 0;

            await answer.update({...updateData, score});

            // Compute and update exam record after answer update
            await this.computeAndUpdateExamRecord(answer.mockQuestionId, answer.userId);
     ...
     */
    private async computeAndUpdateExamRecord(mockQuestionId: string, userId: string): Promise<void> {
        // Get the mock question to find the mockExamId
        const mockQuestion = await this.studentsMockAnswersModel.sequelize.models.MockQuestions.findByPk(mockQuestionId);
        if (!mockQuestion) return;
        // Use getDataValue to access property
        const mockExamId = mockQuestion.getDataValue('mockExamId');

        // Get all questions for the exam
        const allQuestions = await this.studentsMockAnswersModel.sequelize.models.MockQuestions.findAll({ where: { mockExamId } });
        ...
        // Upsert exam record
        const examRecordModel = this.studentsMockAnswersModel.sequelize.models.MockExamRecord;
    ...
    async delete(id: string): Promise<boolean> {
        try {
            const deletedCount = await this.studentsMockAnswersModel.destroy({ where: { id } });
    ...
    async findByUserId(userId: string, offset: number, limit: number): Promise<{ count: number; rows: StudentsMockAnswers[] }> {
        try {
            return await this.studentsMockAnswersModel.findAndCountAll({ where: { userId }, offset, limit });
        } catch (error) {
            throw new BadRequestException({
