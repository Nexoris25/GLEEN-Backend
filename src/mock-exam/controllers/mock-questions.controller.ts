...
import {
  MockQuestionsArrayResponseDto,
  MockQuestionsResponseCountDto,
  MockQuestionsResponseDto,
  ResponseDto,
...
export class MockQuestionController {
  ...
  @ApiResponse({
    status: 201,
    description: 'The mock question has been successfully created.',
    type: MockQuestionsResponseDto,
  })
  @ApiResponse({
    status: 500,
    description: 'Error creating mock question',
    type: ResponseDto<null>,
  })
  async create(
    @Body() createDto: CreateMockQuestionDto,
    @UserId() userId: string,
  ): Promise<MockQuestionsResponseDto> {
    try {
  ...
  @Post('bulk/:mockExamId')
  @ApiOperation({ summary: 'Create multiple mock questions' })
  @ApiParam({ name: 'mockExamId', description: 'Mock Exam ID' })
  @ApiBody({ type: CreateBulkMockQuestionDto })
  @ApiResponse({
    status: 201,
    description: 'The mock questions have been successfully created.',
    type: MockQuestionsArrayResponseDto,
  })
  @ApiResponse({
    status: 500,
    description: 'Error creating mock questions',
    type: ResponseDto<null>,
  })
  async createBulk(
    @Param('mockExamId') mockExamId: string,
    @Body() createBulkDto: CreateBulkMockQuestionDto,
    @UserId() userId: string,
  ): Promise<MockQuestionsArrayResponseDto> {
    try {
      const questions = await this.mockQuestionsService.createBulk(
        createBulkDto,
        mockExamId,
        userId,
      );
      return {
        status: 201,
        data: questions,
        message: 'Mock questions created successfully',
      };
    } catch (error) {
      return {
        status: 500,
        message: 'Error creating mock questions',
        error: stringify({
          message: error.message,
          stack: error.stack,
          details: error.response || error,
  ...
  @ApiResponse({
    status: 200,
    description: 'List of mock questions',
    type: MockQuestionsResponseCountDto,
  })
  @ApiResponse({
    status: 500,
    description: 'Error fetching mock questions',
    type: ResponseDto<null>,
  })
  async findAll(
  ...
  @ApiResponse({
    status: 200,
    description: 'The mock question',
    type: MockQuestionsResponseDto,
  })
  @ApiResponse({
    status: 404,
    description: 'Mock question not found',
    type: ResponseDto<null>,
  })
  @ApiResponse({
    status: 500,
    description: 'Error fetching mock question',
    type: ResponseDto<null>,
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'The mock question has been successfully updated.',
    type: MockQuestionsResponseDto,
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiResponse({
    status: 500,
    description: 'Error updating mock question',
    type: ResponseDto<null>,
  })
  async update(
    @Param('id') id: string,
    @Body() updateDto: UpdateMockQuestionDto,
  ): Promise<MockQuestionsResponseDto> {
    try {
  ...
  @ApiResponse({
    status: 200,
    description: 'The mock question has been successfully deleted.',
    type: ResponseDto<null>,
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiResponse({
    status: 500,
    description: 'Error deleting mock question',
    type: ResponseDto<null>,
  })
