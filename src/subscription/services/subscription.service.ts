...
export class SubscriptionService {
  ...
  return { count, rows };
    ...
    async findAll(userId: string, offset = 0, limit = 10): Promise<{ rows: Subscription[]; count: number }> {
        try {
    ...
    async update(id: string, dto: UpdateSubscriptionDto, userId: string): Promise<Subscription> {
    ...
    async remove(id: string, userId: string): Promise<void> {
        const deleted = await this.subscriptionModel.destroy({ where: { id, userId } });
        if (!deleted) {
            throw new NotFoundException(`Subscription ${id} not found or not allowed`);
        }
    }
}