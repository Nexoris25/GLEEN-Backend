...
@ApiOperation({ summary: 'List PAID subscription transactions, admin and super admin' })
...
@Get('pending')
@Roles('ADMIN', 'SUPER_ADMIN')
@ApiOperation({ summary: 'List PENDING subscription transactions, admin and super admin' })
async pending() {
return {
status: HttpStatus.OK,
...
@Get('failed')
@Roles('ADMIN', 'SUPER_ADMIN')
@ApiOperation({ summary: 'List FAILED subscription transactions, admin and super admin' })
async failed() {
return {
status: HttpStatus.OK,
...
@Roles('ADMIN', 'SUPER_ADMIN')
@ApiOperation({
summary: 'List ABANDONED subscription transactions (pending > 30 mins), admin and super admin',
})
async abandoned() {
return {
status: HttpStatus.OK,
...
@Roles('ADMIN', 'SUPER_ADMIN')
@ApiOperation({
summary: 'Get subscription transactions (optional filters). admin, super admin',
})
...
@Get('me')
@ApiOperation({ 
summary: 'Get subscription transactions for logged-in user; ?current=true returns active subscription',
})
