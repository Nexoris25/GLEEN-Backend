...
export class RoomsService {
  ...
  async create(dto: CreateRoomDto) {
    ...
    let dailyRoom;
    try {
      const dailyResponse = await axios.post(this.DAILY_API_URL, payload, {
        headers: {
...
*/
  ...
  async deleteAllDailyRooms() {
    ...
    do {
      ...
      for (const room of rooms) {
        try {
          console.log(`${this.DAILY_API_URL}/${room.name}`);
          ...
          if (err.response?.status === 404) {
            console.warn(
              `Room ${room.name} (${room.name}) not found on Daily.co — skipping`,
            );
          } else {
            console.error(
              `Failed to delete room ${room.name} (${room.name}):`,
              err.message,
            );
          }
  ...
  async removeByName(name: string) {
    ...
    try {
      ...
      if (err.response?.status !== 404) {
        throw new BadRequestException('Failed to delete room on Daily.co');
      }
    }
    // }
