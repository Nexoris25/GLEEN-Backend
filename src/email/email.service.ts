...
export class MailService {
  ...
  async sendEmailVerificationOtp(arg0: { userEmail: string; otp: string }) {
    console.log(
      'Sending email verification OTP to:',
      arg0.userEmail,
      'OTP:',
      arg0.otp,
    );
    ...
    try {
      ...
      Logger.error(
        `Error sending email to: ${arg0.userEmail}`,
        JSON.stringify(
          {
            message: error.message,
            stack: error.stack,
  ...
  async sendVerificationEmail(metadata: { userEmail: string; link: string }) {
    ...
    if (!metadata.link) {
      Logger.error(
        `Cannot send email: "link" is missing for user ${metadata.userEmail}`,
      );
      return;
    }
    ...
    try {
      ...
      // Catch Handlebars template errors or transport errors
      Logger.error(
        `Error sending email to: ${metadata.userEmail}`,
        JSON.stringify(
          {
            message: error.message,
            stack: error.stack,
